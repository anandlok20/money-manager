...
function getEncryptionKey(): Buffer {
  ...
  if (!keyHex || keyHex.length !== 64) {
    throw new Error(
      'DATA_ENCRYPTION_KEY must be set to a 64-character hex string (32-byte AES key) before storing card CVV/PIN. Generate one with: node -e "console.log(require(\'crypto\').randomBytes(32).toString(\'hex\'))"'
    );
  }
...
function encryptField(plaintext: string): string {
  const key = getEncryptionKey(); // throws if missing — refuse to store plaintext
...
function decryptField(ciphertext: string): string {
  ...
  const key = getEncryptionKey(); // throws if missing
